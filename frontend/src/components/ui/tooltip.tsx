...
const TooltipProvider = ({ delayDuration, ...props }: TooltipPrimitive.Provider.Props & { delayDuration?: number }) => <TooltipPrimitive.Provider delay={delayDuration} {...props} />;
...
const Tooltip = ({ openDelay: _openDelay, closeDelay: _closeDelay, ...props }: LegacyTooltipProps) => (
...
const TooltipTrigger = React.forwardRef<HTMLElement, TooltipTriggerProps>(({ asChild, children, ...props }, ref) => (
  <TooltipPrimitive.Trigger ref={ref} render={asChild ? children : undefined} {...props}>{asChild ? undefined : children}</TooltipPrimitive.Trigger>
));
...
>(({ className, sideOffset = 4, side, align, alignOffset, ...props }, ref) => (
  <TooltipPrimitive.Portal>
    <TooltipPrimitive.Positioner className="isolate z-50" side={side} sideOffset={sideOffset} align={align} alignOffset={alignOffset}>
