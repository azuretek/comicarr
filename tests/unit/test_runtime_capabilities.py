...
def test_runtime_capabilities_report_an_available_unrar_without_running_pip(tmp_path, monkeypatch):
    ...
    with patch("comicarr.dependency_check.shutil.which", return_value="/usr/local/bin/unrar") as which:
        diagnostics.loaders()
    ...
    assert "pip list" not in (Path(__file__).resolve().parents[2] / "comicarr/dependency_check.py").read_text()
